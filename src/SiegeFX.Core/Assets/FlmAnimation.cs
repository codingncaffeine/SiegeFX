...
public static class FlmAnimation
{
    ...
    /// <summary>Returns one BGRA8888 buffer per frame (top-down, ready to
    /// upload as a 32x32 GL texture). Empty array if the file is shorter
    /// than the header or doesn't carry at least one full frame.</summary>
    public static byte[][] LoadFrames(byte[] bytes)
    {
        ...
        for (int i = 0; i < frameCount; i++)
        {
            ...
            Buffer.BlockCopy(bytes, HeaderBytes + i * FrameBytes, buf, 0, FrameBytes);
