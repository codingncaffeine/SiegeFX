...
All **81 shipped DS1 regions** load through the cross-boundary streamer, with **50,680 static-prop placements** texturing 100% via the runtime resolver. NPCs, dialogue, quests, vendors, spells, leveling, audio, and save/load are all in. The render hot path is zero-alloc per actor per frame, and a headless `siegefx balance curve` walks the four skill curves L1→L50 against the shipped `formulas.gas` and confirms HP/MP/XP all grow monotonically. The ASP parser now tracks per-submesh `(firstTri, triCount, textureIndex)` subsets — confirmed across all 2,626 shipped meshes — so multi-texture characters (farmboy = skin + clothing across 5 subsets) no longer lose their clothing texture. End-to-end Farmhouse → Castle Ehb playtest is next.
